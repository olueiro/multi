...
local function bind(text, options)
    ...
    local function insert(current)
        ...
        elseif current == CODE then
            ...
            if not status then
                table.insert(errors, {
                    message = locale(options, "Syntax error in file '%s', line %s (%s)", options.file == "" and locale

(options, "main") or options.file, index - #content, message),
                    file = options.file,
                    line = index - #content
                })
            end
...
function multi:run(user)
    ...
    elseif last.status == REPLIED then
        ...
        if (not passage_index) or (heading_next_index and passage_index > heading_next_index) then
            self.options.events["notice"](self, user, locale(self.options, "Passage '%s' not found in section '%s'", 

last.passage, last.heading))
            return
        end
